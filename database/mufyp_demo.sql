...
-- 為 post_comments 表添加安全檢查相關欄位
ALTER TABLE `post_comments` 
ADD COLUMN `safety_score` FLOAT DEFAULT 0,
ADD COLUMN `is_approved` TINYINT(1) DEFAULT 1,
ADD COLUMN `moderation_reason` TEXT,
ADD COLUMN `moderated_at` TIMESTAMP NULL;

-- 為 posts 表也添加安全檢查欄位（可選）
ALTER TABLE `posts` 
ADD COLUMN `safety_score` FLOAT DEFAULT 0,
ADD COLUMN `is_approved` TINYINT(1) DEFAULT 1;

-- 創建索引以便快速查詢
CREATE INDEX `idx_post_comments_approved` ON `post_comments` (`post_id`, `is_approved`, `created_at`);
CREATE INDEX `idx_posts_approved` ON `posts` (`user_id`, `is_approved`, `created_at`);