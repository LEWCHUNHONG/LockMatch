...
import { View, Text, TextInput, TouchableOpacity, FlatList, StyleSheet, Alert, ActivityIndicator, KeyboardAvoidingView, Platform, Keyboard } from 'react-native';
...
export default function InstantChat() {
    ...
    const getCurrentUserAndSetup = async () => {
        try {
            ...
            console.log('✅ 當前用戶ID (字串):', userIdStr);
            setMyId(userIdStr);
        ...
        timerRef.current = setInterval(() => {
            setTimeLeft(prev => {
                if (prev <= 1) {
                    ...
                    Alert.alert('聊天結束', '1分鐘已到，將返回附近的人頁面', [
                        { text: '確定', onPress: () => router.replace('/(tabs)/location-checkin') }
                    ]);
                    return 0;
                }
                return prev - 1;
    ...
    const setupSocket = (currentUserId) => {
        ...
        socket.on('instant-chat-message', (data) => {
            console.log('📩 收到原始訊息:', data);
            // 確保 senderId 為字串，與 myId 型別一致
            const message = {
            ...
            if (String(data.userId) !== currentUserId) {
                setTypingUsers(prev => {
                    ...
                    if (data.isTyping && !exists) {
                        ...
                        return prev.filter(u => String(u.userId) !== String(data.userId));
                    }
                    return prev;
                });
            }
