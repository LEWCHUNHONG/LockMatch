...
export default function LocationCheckin() {
    ...
    const getCurrentLocationAndNearby = async (showLoading = true) => {
        try {
            ...
            const newLocation = {
                ...
                latitudeDelta: 0.02, // 稍微放大視野顯示附近用戶
                longitudeDelta: 0.02,
            };
    ...
    const startChat = async (targetUserId) => {
        try {
            const response = await api.post('/api/chat/create-temp-chat', { targetUserId });
            if (response.data.success) {
                router.push(`/chat/${response.data.roomId}`);
            } else {
                Alert.alert('錯誤', response.data.error || '無法開始對話');
            }
        } catch (error) {
            Alert.alert('錯誤', error.response?.data?.error || '請稍後再試');
        }
    };



    // 在 renderAvatarMarker 中，傳入 username
    const renderAvatarMarker = (user) => (
        <Marker
            key={user.id}
            coordinate={{ latitude: user.latitude, longitude: user.longitude }}
            onPress={() => sendTempChatInvite(user.id, user.username)} // 傳入 username
        >
