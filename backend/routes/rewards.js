    ...
    if (userTaskResults.length === 0) {
      ...
      await connection.promise().query(
        `INSERT INTO user_tasks (user_id, task_id, user_status, current_progress, started_at, completed_at) 
         VALUES (?, ?, ?, ?, NOW(), ?)`,
        [userId, taskId, isCompleted ? 'completed' : 'in_progress', currentProgress, isCompleted ? new Date() : null]
      );
    } else {
      // 更新現有記錄
      await connection.promise().query(
        ...
        [isCompleted ? 'completed' : 'in_progress', currentProgress, isCompleted ? new Date() : null, userId, taskId]
      );
    }
    
    // 5. 如果任務完成，發放獎勵
    if (isCompleted && (userTaskResults.length === 0 || userTaskResults[0].user_status !== 'completed')) {
...
router.get('/tasks', authMiddleware(JWT_SECRET), async (req, res) => {
  ...
  try {
    ...
    const grouped = { initial: [], achievement: [] };
    
    // 對每個任務進行動態檢查
    for (const task of tasks) {
      // 檢查並更新任務狀態
      await checkAndCompleteTask(userId, task.title);
...
router.get('/init-rewards-db', authMiddleware(JWT_SECRET), (req, res) => {
  const queries = [
    ...
    `CREATE TABLE IF NOT EXISTS tasks (
      ...
      description TEXT,
      task_type ENUM('initial', 'achievement', 'special') DEFAULT 'initial',
